...
def transform():
    ...
    log(logfile, "Transformacion Iniciada")
    ...
    df_employees = pd.read_sql_query("""SELECT 
        ...
        PostalCode,  Phone, Fax, Email  FROM employees;
        """, con=engine.connect())
    ...
    df_location = pd.read_sql_query("""SELECT 
        CustomerId as LocationId,   Address,  City, 
        COALESCE(State, 'N/A') as State,  Country, COALESCE(PostalCode, 'N/A') as PostalCode 
        FROM customers; """, con=engine.connect())

    log(logfile, "Transformacion Finalizada")
...
def extract():
    log(logfile, "Extracion Finalizada")
    metadata = MetaData()
    metadata.create_all(engine)
    log(logfile, "Extraccion Finalizada")
...
if __name__ == '__main__':
    ...
    extract()
    (df_albums,  df_artists, df_customers, 
    df_employees,  df_genres,  df_invoice_items, 
    df_location, df_playlists) = transform()
    load()
